import type { FeatureContext, FeatureFlags, SessionData, FeatureSDKConfig } from './types.js';
...
export class FeatureSDKClient {
  ...
  constructor(config: FeatureSDKConfig = {}) {
   ...
   */
  async isFeatureEnabled(feature: string, context?: Partial<FeatureContext>): Promise<boolean> {
   ...
   */
  async getFeatures(context?: Partial<FeatureContext>): Promise<FeatureFlags> {
    // Use session data if available
   ...
   */
  private async queryFeatureFlag(feature: string, context?: Partial<FeatureContext>): Promise<boolean> {
    try {
   ...
   */
  private getCacheKey(feature: string, context?: Partial<FeatureContext>): string {
    const ctx = context || {};
   ...
   */
  getCurrentContext(): FeatureContext {
    if (!this.sessionData) {
      return {};
    }
